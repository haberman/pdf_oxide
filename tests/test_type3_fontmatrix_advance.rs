...
fn type3_pdf_no_widths_array(font_matrix_a: f64, tfs: f64) -> Vec<u8> {
    ...
    // d1 declares width + bounding box; the fill rect guarantees a renderable mark
    // so extract_chars will always surface this glyph.
    let char_proc_a = "5 0 0 0 5 5 d1\n0 0 5 5 re f\n".to_string();
...
fn type3_pdf_advance(font_matrix_a: f64, glyph_width: f64, tfs: f64) -> Vec<u8> {
    ...
    // d1 declares width + bounding box; the fill rect guarantees a renderable mark
    // so extract_chars will always surface this glyph.
    let char_proc_a = format!("{glyph_width} 0 0 0 {glyph_width} {glyph_width} d1\n0 0 {glyph_width} {glyph_width} re f\n");
...
fn type3_fontmatrix_advance_width_scaled_correctly() {
    ...
    let ch = chars
        .iter()
        .find(|c| c.char == 'A')
        .expect("Type3 'A' should be extracted — CharProc uses d1 with a fill rect");
